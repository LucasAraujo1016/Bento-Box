...
export default class Receitas extends Component<any, State> {
    ...
    limparFiltros = () => {
        this.setState({ filtroHabilidade: [], filtroCulinaria: [], filtroRestricoes: [] });
    }
    ...
    obterReceitasFiltradas = () => {
        ...
        if (filtroHabilidade.length > 0) {
            const vals = filtroHabilidade.map(f => f === "Intermediário" ? "intermediario" : f.toLowerCase());
            resultado = resultado.filter(r => r.nivelHabilidade && vals.includes(r.nivelHabilidade.toLowerCase()));
        }
        if (filtroCulinaria.length > 0) {
            const vals = filtroCulinaria.map(f => f.toLowerCase());
            resultado = resultado.filter(r => r.tipoCulinaria && vals.includes(r.tipoCulinaria.toLowerCase()));
        }
        if (filtroRestricoes.length > 0) {
            resultado = resultado.filter(r => {
                const rest = r.restricoes || [];
                return filtroRestricoes.every(sel => rest.includes(sel));
            });
        }
        return resultado;
    }
        ...
        return (
            <View style={{ flex: 1, backgroundColor: '#f5f5f5' }}>
                ...
                <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 20 }}>
                    <Pressable
                        style={[style.nova_receita, { marginBottom: 20, backgroundColor: '#FF9D4D', alignSelf: 'center' }]}
                ...
                {/* Overlay flutuante de timers — fica visível mesmo com o modal fechado */}
